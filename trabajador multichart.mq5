...
input group "=== OBJETIVO GLOBAL POR PASOS (STEP PROFIT) ==="
// Al alcanzar el objetivo de EQUITY GLOBAL (suma de todos los pares) se
// cierran TODAS las posiciones del EA, se borran las órdenes límite
// pendientes (reales y virtuales) y TODOS los niveles de par vuelven a 1.
// La referencia es un TRINQUETE que solo sube: si cierra en 1001, la nueva
// base es 1001 y la siguiente meta es 1002, luego 1003... nunca baja.
input bool   InpUseStepProfit     = true;   // Activar cierre global por objetivo
enum ENUM_STEP_TARGET_MODE
{
   STEP_TARGET_USD = 0,   // objetivo fijo en dólares por paso
   STEP_TARGET_PCT = 1    // objetivo en % de la referencia (base trinquete)
};
input ENUM_STEP_TARGET_MODE InpStepTargetMode = STEP_TARGET_USD;
input double InpStepProfitUSD    = 1.0;    // Objetivo por paso en USD (modo USD)
input double InpStepProfitPct    = 0.1;    // Objetivo por paso en % (modo %)
input double InpStepBaseOverride = 0.0;    // Base/referencia manual (0 = equity al arrancar; p.ej. 1000 fija la base en 1000)
...
//+------------------------------------------------------------------+
//| OBJETIVO GLOBAL POR PASOS (STEP PROFIT)                          |
//|   g_StepBaseEquity = referencia trinquete (solo sube). Cuando el  |
//|   equity global llega a base + paso → cierre total, niveles a 1  |
//|   y nueva base = equity resultante.                              |
//+------------------------------------------------------------------+
double   g_StepBaseEquity          = 0.0;
bool     g_StepClosing             = false;  // cierre global en curso (los cierres no mueven niveles)
int      g_StepsClosed             = 0;      // nº de pasos cobrados (informativo)
...
//+------------------------------------------------------------------+
//| OBJETIVO GLOBAL POR PASOS (STEP PROFIT)                          |
//|   · El EQUITY FLOTANTE global (todos los pares sumados) debe      |
//|     subir el importe del paso por encima de la referencia.       |
//|   · Alcanzado: cierra TODAS las posiciones del EA, borra límites  |
//|     reales/virtuales y pone TODOS los niveles de par a 1.         |
//|   · Después fija la referencia en el balance resultante: es un    |
//|     TRINQUETE, nunca baja (1001 → 1002 → 1003...).                |
//|   · Los cierres de la canasta NO mueven niveles ni CV ni sacan a  |
//|     ninguna estrategia de LIVE (el reinicio a 1 manda).           |
//+------------------------------------------------------------------+
double StepProfitAmount()
{
   if(InpStepTargetMode==STEP_TARGET_PCT)
      return g_StepBaseEquity*MathMax(0.0,InpStepProfitPct)/100.0;
   return MathMax(0.0,InpStepProfitUSD);
}

double StepProfitTarget()
{ return g_StepBaseEquity+StepProfitAmount(); }

//--- ¿Hay alguna posición abierta que pertenezca al EA? -------------
bool HasAnyEAPosition()
{
   for(int i=PositionsTotal()-1;i>=0;i--)
   { ulong t=PositionGetTicket(i);
     if(t==0) continue;
     if(!PositionSelectByTicket(t)) continue;
     if(IsAnyMagic((long)PositionGetInteger(POSITION_MAGIC))) return true; }
   return false;
}

//--- Borra TODAS las órdenes pendientes (límites) del EA -----------
void DeleteAllEAPendingOrders()
{
   ...
   { ulong t=OrderGetTicket(i);
     ...
     if(OrderSend(req,res))
        Print("STEP: orden pendiente #",t," [",sym,"] eliminada");
     else
        Print("STEP: no se pudo eliminar la orden #",t," [",sym,"] ret=",res.retcode);
   }
}

//--- Lanza el cierre global por objetivo ----------------------------
void StepProfitTrigger()
{
   double eq =AccountInfoDouble(ACCOUNT_EQUITY);
   double tgt=StepProfitTarget();
   Print("🎯 STEP PROFIT ALCANZADO — Equity ",DoubleToString(eq,2),
         " ≥ meta ",DoubleToString(tgt,2),
         " (base ",DoubleToString(g_StepBaseEquity,2)," + paso ",
         DoubleToString(StepProfitAmount(),2),
         InpStepTargetMode==STEP_TARGET_PCT?" USD (%)":" USD",
         ") → cierre GLOBAL de todos los pares");
   g_StepClosing=true;
   ...
   { for(int st=0;st<STRAT_COUNT;st++)
     ...
     //--- esperas/límites congelados de confluencia fuera
...
//--- Remata el reinicio cuando los cierres ya están liquidados ------
void StepProfitFinalize()
{
   for(int si=0;si<g_SymCount;si++)
      g_PairLevel[si]=1;                 // tabla de riesgo de todos los pares a nivel 1

   //--- Nueva base trinquete: balance/equity resultante, nunca por
   //    debajo de la base anterior.
   double bal=AccountInfoDouble(ACCOUNT_BALANCE);
   double eq =AccountInfoDouble(ACCOUNT_EQUITY);
   double ref=MathMax(bal,eq);
   if(ref<g_StepBaseEquity) ref=g_StepBaseEquity;
   g_StepBaseEquity=ref;
   g_StepsClosed++;
   g_StepClosing=false;

   Print("✅ STEP PROFIT #",g_StepsClosed," cobrado — TODOS los NIVELES a 1.",
         " Nueva base=",DoubleToString(g_StepBaseEquity,2),
         " | próxima meta=",DoubleToString(StepProfitTarget(),2),
         " (paso ",DoubleToString(StepProfitAmount(),2)," USD)");
   SaveState();
   if(!IsTester()) RebuildPanel();
}

//--- Llamada cada tick desde OnTick (tras procesar cierres) ---------
void CheckStepProfit()
{
   if(!InpUseStepProfit) return;
   if(g_StepBaseEquity<=0.0)
      g_StepBaseEquity=AccountInfoDouble(ACCOUNT_EQUITY);

   if(g_StepClosing)
   { //--- si algún cierre falló (recotización), reintentar cada tick
     if(HasAnyEAPosition())
     { DeleteAllEAPendingOrders();
       CloseAllSymbolsPositions();
       return; }
     //--- esperar a que todos los cierres se liquiden y se procesen
     if(g_ClosedCount==0)
        StepProfitFinalize();
     return; }

   //--- sin posiciones del EA no hay nada que cerrar ni cobrar
   if(!HasAnyEAPosition()) return;
   double eq=AccountInfoDouble(ACCOUNT_EQUITY);
   if(eq>=StepProfitTarget())
      StepProfitTrigger();
}
...
void SaveStateToFile()
{
   ...
   FileWriteString(h,"STEP_BASE_EQ="  +DoubleToString(g_StepBaseEquity,8)+"\n");
   FileWriteString(h,"STEP_CLOSED="   +IntegerToString(g_StepsClosed)    +"\n");
   for(int si=0;si<g_SymCount;si++)
...
void LoadStateFromFile()
{
   ...
   { string line=FileReadString(h);
     ...
     else if(key=="STEP_BASE_EQ"){ double sb=StringToDouble(val); if(sb>0.0) g_StepBaseEquity=sb; }
     else if(key=="STEP_CLOSED")    g_StepsClosed=(int)MathMax(0,StringToInteger(val));
     else
     { for(int si=0;si<g_SymCount;si++)
       { string sp="SYM"+IntegerToString(si)+"_";
...
void OpenByStrategy(int si, int st, int signal)
{
   ...
   if(st==STRAT_CONFLUENCIA && !InpAllowConfluOrders) return;
   if(signal==0) return;
   if(g_StepClosing) return;          // cierre global por objetivo en curso
...
void ProcessClosedQueue()
{
   ...
   { ClosedSnap snap=g_ClosedQueue[q]; if(snap.ticket==0) continue;
     ...
     Print("Cierre [",sym,"/",sn,"] #",snap.ticket," PL=",cPL," TP=",hTP," SL=",hSL,
           g_StepClosing?" [STEP → neutral, niveles a 1]":(wc?" [SEMANAL]":""));
     //--- Cierre por objetivo global: NO se aplica SL/TP ni se mueve CV;
     //    la canasta es neutral y StepProfitFinalize pone todos los
     //    niveles a 1 (las estrategias se mantienen LIVE).
     if(g_StepClosing) continue;
...
//+------------------------------------------------------------------+
//| Tira del objetivo global por pasos (base trinquete → meta)       |
//+------------------------------------------------------------------+
#define MP_STEP_H 20
void MPDrawStep(int x,int y,int w)
{
   double eq  =AccountInfoDouble(ACCOUNT_EQUITY);
   double amt =StepProfitAmount();
   double tgt =StepProfitTarget();
   double prog=eq-g_StepBaseEquity;
   double frac=(amt>0.0)?MathMax(0.0,MathMin(1.0,prog/amt)):0.0;

   MPRect(x,y,w,MP_STEP_H,C'12,22,16');
   g_MP.Line(x,y,x+w-1,y,MPC(C'40,70,50'));

   string txt;
   color  clr;
   if(g_StepClosing)
   { txt=StringFormat("STEP #%d  CERRANDO CANASTA…",g_StepsClosed+1);
     clr=clrGold; }
   else
   { string modo=(InpStepTargetMode==STEP_TARGET_PCT)
                 ?StringFormat("%.2f%% = %.2f USD",InpStepProfitPct,amt)
                 :StringFormat("%.2f USD",amt);
     txt=StringFormat("STEP #%d  base %.2f → %.2f   (%s%.2f / %s)",
                      g_StepsClosed+1,g_StepBaseEquity,tgt,
                      (prog>=0.0?"+":""),prog,modo);
     clr=(frac>=1.0)?clrLimeGreen:(prog>=0.0)?clrAqua:clrTomato; }
   MPText(x+8,y+6,txt,clr,true,8);

   //--- barra de progreso (mitad derecha)
   int bx=x+w/2, bw=w/2-16, by=y+12;
   MPRect(bx,by,bw,5,C'30,34,44');
   if(frac>0.0)
   { color fc=(frac>=1.0)?clrLimeGreen:(frac>=0.75)?clrYellow:clrSpringGreen;
     MPRect(bx,by,(int)MathMax(2,MathFloor(bw*frac)),5,fc); }
   MPTextR(x+w-8,y+2,
           g_StepClosing?"…":StringFormat("%.0f%%",frac*100.0),
           C'150,180,150',true,7);
}
...
void MultiPanelUpdate(bool force=false)
{
   ...
   int W=620;
   int stepH=InpUseStepProfit?MP_STEP_H:0;
   int H=22+24+stepH+14+rows*MP_ROW_H;
   if(nAct>rows) H+=13;
   ...
   if(InpUseStepProfit){ MPDrawStep(0,y,W); y+=MP_STEP_H; }
...
void ShowTesterInfo()
{
   ...
   if(InpUseStepProfit)
   {
      ...
      double prog=eqS-g_StepBaseEquity;
      string stTxt=g_StepClosing?"CERRANDO CANASTA…":
         StringFormat("%s%.2f / %.2f",(prog>=0?"+":""),prog,StepProfitAmount());
      msg+=StringFormat("║  STEP objetivo: %.2f -> %.2f  %s\n",
                        g_StepBaseEquity,StepProfitTarget(),stTxt);
   }
   msg+="╠══════════════════════════════════════════╣\n";
   for(int si=0;si<g_SymCount;si++)
   {
...
void PrintDiag()
{
   ...
   Print("=== DIAG v8.38 === X=",InpXActivacion,
         ...
         " Base=",DoubleToString(g_BaseCapital,2),
         InpUseStepProfit?
            StringFormat(" | STEP base=%.2f meta=%.2f eq=%.2f%s (#%d)",
               g_StepBaseEquity,StepProfitTarget(),
               AccountInfoDouble(ACCOUNT_EQUITY),
               g_StepClosing?" CERRANDO":"",g_StepsClosed)
            :" | STEP OFF");
   for(int si=0;si<g_SymCount;si++)
   {
      int alive=g_SysState[si].activeLiveStrategy;
...
int OnInit()
{
   ...
   //--- Objetivo global por pasos: referencia trinquete guardada,
   //    override manual o equity actual si no hay referencia previa.
   if(InpStepBaseOverride>0.0)
      g_StepBaseEquity=InpStepBaseOverride;
   if(g_StepBaseEquity<=0.0)
      g_StepBaseEquity=AccountInfoDouble(ACCOUNT_EQUITY);
   g_StepClosing=false;
   if(InpUseStepProfit)
      Print("STEP PROFIT activo — Base ref=",DoubleToString(g_StepBaseEquity,2),
            " | modo=",(InpStepTargetMode==STEP_TARGET_PCT?"PORCENTAJE":"USD"),
            " | paso=",DoubleToString(StepProfitAmount(),2)," USD",
            " | primera meta=",DoubleToString(StepProfitTarget(),2),
            " | pasos cobrados=",g_StepsClosed);
...
void OnTick()
{
   ...
   ProcessClosedQueue();
   CheckCircuitBreaker();
   CheckStepProfit();         // objetivo global por pasos: cierre total + niveles a 1
   ...
   if(!g_CircuitBreakerOn && !g_StepClosing)
   { EnforceSLTP();
     ManageOpenPositions(); }

   // Virtuales siempre (salvo cbPaused individual o cierre global en curso)
   for(int si=0;si<g_SymCount;si++)
      for(int st=0;st<STRAT_COUNT;st++)
      { if(!g_SysState[si].strategies[st].enabled)  continue;
        if(g_SysState[si].strategies[st].cbPaused)  continue;
   ...
   if(!g_CircuitBreakerOn&&!g_StepClosing&&IsTradeTimeAllowed())
      UpdateAllStrategies();
                  ...
                  const double &dparam,const string &sparam)
{
   ...
   if(sparam==PFX_CFG+"CLR")
   {
      ...
      g_StepClosing     =false;
      g_StepsClosed     =0;
      g_StepBaseEquity  =(InpStepBaseOverride>0.0)?InpStepBaseOverride:
                         AccountInfoDouble(ACCOUNT_EQUITY);
      Print("STEP: estado reiniciado — nueva base ref=",
            DoubleToString(g_StepBaseEquity,2));
      RebuildPanel(); return;
   }
